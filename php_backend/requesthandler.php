    ...
    switch ($action) {
        ...
        case 'update':
            ...
            case 'delete':
                ...
                try {
                    $sql = "UPDATE requests SET status = 0, closure_reason = 'user' WHERE id = :id AND user_id = :user_id";
                    $stmt = $conn->prepare($sql);
                    $stmt->bindParam(':id', $requestId);
                    $stmt->bindParam(':user_id', $userId);
                    $stmt->execute();
                    ...
                    if ($stmt->rowCount() > 0) {
                        $sql = "UPDATE applications SET status = 0, closure_reason = 'system' WHERE request_id = :request_id";
                        $stmt = $conn->prepare($sql);

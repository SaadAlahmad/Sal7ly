...
class ReviewController extends Controller
{
    ...
    public function store(StoreReviewRequest $request) {
        ...
        if($project->status !== 'completed')
            return response()->json(['status' => false, 'message' => 'Project not completed.'], 403);

        $direction = $user instanceof User ? 'client_to_craftsman' : 'craftsman_to_client';

        Review::where('project_id', $project->id)
        ...
        $review->fill($request->only(['rating', 'review_text']));
        $review->save();
